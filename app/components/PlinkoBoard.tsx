...
export function PlinkoBoard() {
  ...
  useEffect(() => {
    ...
    RAPIER.init().then(() => {
      ...
      dropRef.current = () => { if (active || balance < betValue.current) return; const currentBet = betValue.current; setBalance(v => Math.max(0, v - currentBet)); setResult(null); const desc = RAPIER.RigidBodyDesc.dynamic().setTranslation((Math.random() - .5) * .35, 5.8, 0).setLinvel(0, 0, 0); desc.setCcdEnabled(true).setCanSleep(false).setLinearDamping(.08); body = world.createRigidBody(desc); body.setEnabledTranslations(true, true, false, true); body.lockRotations(true, true); world.createCollider(RAPIER.ColliderDesc.ball(ballRadius).setRestitution(.82).setFriction(.18).setDensity(1.2), body); ball.visible = true; active = true; };
      let lastTime = performance.now();
      const tick = (now = performance.now()) => { const elapsed = Math.min((now - lastTime) / 1000, .05); lastTime = now; const steps = Math.max(1, Math.min(4, Math.ceil(elapsed / (1 / 60)))); for (let step = 0; step < steps; step++) world.step(); if (body) { const p = body.translation(); ball.position.set(p.x, p.y, p.z); ball.quaternion.set(body.rotation().x, body.rotation().y, body.rotation().z, body.rotation().w); if (p.y < -3.7) { const index = Math.max(0, Math.min(6, Math.floor((p.x + 4.9) / 1.4))); const win = Math.round(betValue.current * multipliers[index]); setResult(`${pockets[index]}  /  +${win}`); setBalance(v => v + win); world.removeRigidBody(body); body = null; active = false; if (autoValue.current) window.setTimeout(() => dropRef.current?.(), 450); } } renderer.render(scene, camera); frame = requestAnimationFrame(tick); };
      tick();
